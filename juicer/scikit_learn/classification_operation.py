...
class ClassifierModel(Operation):
    FEATURES_PARAM = 'features'
    LABEL_PARAM = 'label'
    PREDICTION_ATTR_PARAM = 'prediction'
    INPUT_PORT = 'train input data'
    ...
    def __init__(self, parameters, named_inputs, named_outputs):
        ...
        self.output = named_outputs.get(
            'algorithm', 'classifier_algorithm_{}'.format(self.order))
        self.model = self.named_outputs.get('model', 'model_tmp')
        self.input = self.named_inputs.get(self.INPUT_PORT)

    def read_common_params(self, parameters):
        self.has_code = self.INPUT_PORT in self.named_inputs
        ...
        if not all([self.LABEL_PARAM in parameters,
            ...
            self.prediction = parameters.get(
                    self.PREDICTION_ATTR_PARAM, 'prediction')
...
class DecisionTreeClassifierModelOperation(ClassifierModel):
    ...
    def __init__(self, parameters,  named_inputs, named_outputs):
        ClassifierModel.__init__(self, parameters,  named_inputs,  named_outputs)

        self.has_code = any([len(named_outputs) > 0, self.contains_results()])
        self.name = 'classifier.DecisionTreeClassifierModelOperation'
    ...
    def generate_code(self):
        ...
        min_samples_split={min_split}, min_samples_leaf={min_leaf}, 
        min_weight_fraction_leaf={min_weight}, random_state={seed}).fit(X, y)
...
class GBTClassifierModelOperation(ClassifierModel):
    ...
    def __init__(self, parameters,  named_inputs, named_outputs):
        ...
        self.has_code = any([len(named_outputs) > 0, self.contains_results()])
        self.parameters = parameters
        self.name = 'classifier.GBTClassifierModelOperation'
        if self.has_code:
            self.read_common_params(parameters)
            self.max_depth = parameters.get(
                    self.MAX_DEPTH_PARAM, 3) or 3
    ...
    def generate_code(self):
        ...
        learning_rate={learning_rate}, n_estimators={n_estimators},
        min_samples_split={min_split}, max_depth={max_depth},
        min_samples_leaf={min_leaf}, random_state={seed}).fit(X, y)
        
        {output} = {input}.copy()
        {output}['{prediction}'] = {model}.predict(X).tolist()
        """.format(output=self.output, model=self.model, label=self.label,
                   features=self.features, prediction=self.prediction,
                   input=self.input,
                   loss=self.loss,
                   n_estimators=self.n_estimators, min_leaf=self.min_leaf,
...
class KNNClassifierModelOperation(ClassifierModel):
    ...
    def __init__(self, parameters,  named_inputs, named_outputs):
        ...
        self.has_code = any([len(named_outputs) > 0, self.contains_results()])
        self.name = 'classifier.KNNClassifierModelOperation'
        self.parameters = parameters
...
class LogisticRegressionModelOperation(ClassifierModel):
    ...
    def __init__(self, parameters,  named_inputs, named_outputs):
        ...
        if self.has_code:
            self.read_common_params(parameters)
            self.name = 'classifier.LogisticRegressionModelOperation'
            self.tol = self.parameters.get(self.TOLERANCE_PARAM,
                                           0.0001) or 0.0001
            self.tol = abs(float(self.tol))
            self.regularization = self.parameters.get(self.REGULARIZATION_PARAM,
    ...
    def generate_code(self):
        ...
        code = """
            ...
            solver='{solver}', random_state={seed}).fit(X, y)
...
class MLPClassifierModelOperation(ClassifierModel):
    ...
    def __init__(self, parameters,  named_inputs, named_outputs):
        ...
        self.has_code = any([len(named_outputs) > 0, self.contains_results()])
        if self.has_code:
            self.name = 'classifier.MLPClassifierModelOperation'
            self.read_common_params(parameters)
            self.hidden_layers = parameters.get(self.HIDDEN_LAYER_SIZES_PARAM,
                                                '(1,100,1)') or '(1,100,1)'
            self.hidden_layers = \
    ...
    def generate_code(self):
        ...
        code = """
            ...
            activation='{activation}', solver='{solver}', alpha={alpha}, 
            max_iter={max_iter}, random_state={seed}, tol={tol}).fit(X, y)
...
class NaiveBayesClassifierModelOperation(ClassifierModel):
    ...
    def __init__(self, parameters,  named_inputs, named_outputs):
        ...
        self.has_code = any([len(named_outputs) > 0, self.contains_results()])
        if self.has_code:
            self.name = 'classifier.NaiveBayesClassifierModelOperation'
            self.read_common_params(parameters)
            self.class_prior = parameters.get(self.CLASS_PRIOR_PARAM,
                                              'None') or 'None'
            if self.class_prior != "None":
    ...
    def generate_code(self):
        ...
        if self.model_type == self.MODEL_TYPE_PARAM_M:
            ...
            {model} = MultinomialNB(alpha={alpha}, class_prior={prior}).fit(X, y)
            ...
            {model} = BernoulliNB(alpha={smoothing}, class_prior={prior}).fit(X, y)
            ...
            """.format(output=self.output, model=self.model,
                       ...
                       features=self.features, prediction=self.prediction,
                       input=self.input,
                       smoothing=self.smoothing, prior=self.class_prior)
        else:
            code = """
            X = {input}['{features}'].values.tolist()
            y = {input}['{label}'].values.tolist()
            
            {model} = GaussianNB(prior={prior}).fit(X, y)
...
class PerceptronClassifierModelOperation(ClassifierModel):
    ...
    def __init__(self, parameters,  named_inputs, named_outputs):
        ...
        self.has_code = any([len(named_outputs) > 0, self.contains_results()])
        if self.has_code:
            self.name = 'classifier.PerceptronClassifierModelOperation'
            self.read_common_params(parameters)
            self.max_iter = parameters.get(self.MAX_ITER_PARAM, 1000) or 1000
            self.alpha = parameters.get(self.ALPHA_PARAM, 0.0001) or 0.0001
            self.tol = parameters.get(self.TOLERANCE_PARAM, 0.001) or 0.001
    ...
    def generate_code(self):
        ...
        {model} = Perceptron(tol={tol}, alpha={alpha}, max_iter={max_iter},
        shuffle={shuffle}, random_state={seed}, penalty='{penalty}').fit(X, y)
...
class RandomForestClassifierModelOperation(ClassifierModel):
    ...
    def __init__(self, parameters,  named_inputs, named_outputs):
        ...
        self.has_code = any([len(named_outputs) > 0, self.contains_results()])
        if self.has_code:
            self.name = 'classifier.RandomForestClassifierModelOperation'
            self.read_common_params(parameters)
            self.seed = parameters.get(self.SEED_PARAM, 'None') or 'None'
            self.min_split = parameters.get(self.MIN_SPLIT_PARAM, 2) or 2
            self.min_leaf = parameters.get(self.MIN_LEAF_PARAM, 1) or 1
...
class SvmClassifierModelOperation(ClassifierModel):
    ...
    def __init__(self, parameters,  named_inputs, named_outputs):
        ...
        self.has_code = any([len(named_outputs) > 0, self.contains_results()])
        if self.has_code:
            self.name = 'classifier.SvmClassifierModelOperation'
            self.read_common_params(parameters)
            self.max_iter = parameters.get(self.MAX_ITER_PARAM, -1)
            self.tol = parameters.get(self.TOLERANCE_PARAM, 0.001) or 0.001
            self.tol = abs(float(self.tol))
    ...
    def generate_code(self):
        ...
        """.format(output=self.output, model=self.model,
                   ...
                   prediction=self.prediction,
                   input=self.input,
                   tol=self.tol, c=self.c, max_iter=self.max_iter,
                   degree=self.degree, kernel=self.kernel, seed=self.seed
                   )
        return dedent(code)