...
class STDBSCANOperation(Operation):
    ...
    def generate_code(self):
        ...
        {output} = st_dbscan.fit_transform({data_input}, col_lat='{col_latitude}', col_lon='{col_longitude}',
...
class CartographicProjectionOperation(Operation):
    ...
    def __init__(self, parameters, named_inputs, named_outputs):
        ...
        if self.has_code:
            ...
            for att in [self.SRC_PROJ_PARAM, self.DST_PROJ_PARAM, self.LAT_PARAM, self.LON_PARAM]:
                if att not in self.parameters:
                    raise ValueError(
                        _('Parameters {} must be informed for task {}.').format(att, self.__class__))
